...
const Testimonial = () => {
  ...
  return (
    ...
    >
      <article className="container mx-auto bg-secondary relative py-14 p-4 px-5 max-w-[1200px] flex flex-col items-center justify-center gap-4">
        <div className="flex flex-col items-center justify-center">
          <h2 className="text-h2 lg:text-h3 font-bold text-center max-w-[80%]">What Early Users Are Saying</h2>
          ...
          <article className="flex flex-col items-center justify-center mt-10">
            <p className="mt-5 text-center max-w-[80%] lg:max-w-[60%] lg:leading-8">
              Our beta testers have some wonderful experience to share with you. Here are some of the reviews we have received from them.
            </p>
          </article>
        </div>

        <div className="mt-4 bg-secondary w-full relative">
          <motion.article
            initial={{ opacity: 0, y: 100 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.3, delay: 0.2 }}
            viewport={{ once: true }}
            className="absolute w-[100%] bg-white top-[3rem] left-0 right-0"
          >
            <div className="relative w-full mb-20">
              <div className="embla" ref={emblaRef}>
                <div className="embla__container">
                  {Reviews.map((review, index) => (
                    <div key={index} className="embla__slide px-4 md:max-w-[60%]">
                      <div className="bg-white border rounded-lg py-12 px-5 shadow-xl relative items-center flex flex-col gap-6 h-full">
                        ...
                        <p className="text-[#505050] leading-relaxed text-center">{review.desc}</p>
                        <div className="flex items-center gap-4">
                          <div className="flex gap-2 items-start">
                            <h1 className="text-primary font-extrabold text-[16px] leading-[24.55px]">
                              {`- ${review.name},`}
                            </h1>
                            <span className="text-[#091A34] italic">{review.title}</span>
                          </div>
                        </div>
                      </div>
                    </div>
