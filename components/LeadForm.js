  ...
  return (
    ...
    >
      ...
      <form
        ...
        className="relative flex flex-col items-center py-14 gap-4 bg-primary text-white w-[90%] max-w-[500px] p-6 rounded-lg"
      >
        <h2 className="text-xl font-semibold">Join Our Waitlist</h2>
        <p className="text-sm text-center md:max-w-[85%] lg:max-w-[60%] text-silver mb-2">
          Enter your valid email to get a special discount when we go live!
        </p>
        <input
          type="email"
          name="email"
      ...
      {success && <Success closeForm={handleClose} />}
    </motion.div>
  );
}
